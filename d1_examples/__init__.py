...
ENVIRONMENTS = {
    "production": {
        "home": os.path.join(DATAONE_HOME, "env", "production"),
    ...
    "stage": {
        "home": os.path.join(DATAONE_HOME, "env", "stage"),
...
def create_app(test_config=None):
    ...
    def parseResourceMap():
        ...
        if request.method == "GET":
            ...
            if target == "":
                return sendError(
                    "/oreparse/", "GET", "No target resource in request path", 404
                )
            logging.debug("Requesting URL = %s", target)
            cert = None
            try:
                ...
                return sendError(
                    "/oreparse/", "GET", "Request to source timed out", 504
                )
            headers = dict(r.headers)
            if r.status_code in [302, 303]:
                r.status_code = 200
            ...
            if ore_doc is None:
                return sendError(
                    "/oreparse/", "POST", "Expected RDF-XML in ore POST parameter.", 400
                )
